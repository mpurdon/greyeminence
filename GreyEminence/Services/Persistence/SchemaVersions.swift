...
/// SchemaV21 adds `TranscriptSegment.micLevel` — loudness of the mic audio
/// under a line, for telling speaker bleed from the user's voice. One
/// optional attribute — lightweight migration; existing rows get nil.
...
/// SchemaV24 adds `Meeting.refinementLikelihood` / `refinementFeature` /
/// `refinementOverride` — whether the meeting looks like a feature
/// refinement (from the final analysis, or the backfill for older meetings)
/// and the user's manual add/remove. Three optional attributes — lightweight
/// migration; existing meetings get nil, which the backfill picks up.
...
/// SchemaV25 adds `Meeting.refinementFeatures` — every feature a meeting
/// refined, not just the first. One defaulted array — lightweight
/// migration; existing meetings get [], which the backfill fills in.
